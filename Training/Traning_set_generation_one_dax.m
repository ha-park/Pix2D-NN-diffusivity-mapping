...
Frame_length    = 200;               %simulation movie length
sim_t           = 1/100;           %simulation time interval in ms
photon_burst    = 80;               %photon per molecule per frame per millisecond
background      = [1100 150];
diffusivity     = (0:0.02:6)';
pixel_length    = 160; 
...
for i = 1:1
    ...
    DaxNameHead     = sprintf('%dHz_bg%d_photon %d_%dto%d',round(1000/exposure_t(i)),background(1),photon_burst,diffusivity(1),diffusivity(end));  %not include .dax in the string
    for d = 1:D_num
        tempdax = FreeDmap_to_Dax_trainingset_oneDax_gaussian_background(diffusivity(d), ...
            dim, Frame_length, particle_density, exposure_t(i), sim_t, photon_burst, background, pixel_length/1000, sig);

function [roi_array,roi_xy,dim] = Extract_roi_avoid_vicinity_norm_insightbg_v3(dax_name,mlist_name,roi_width,frame_interval)
    ...
    rx = ceil((r.x)-0.5); ry = ceil((r.y)-0.5); background = r.bg;    
    ...
    for f = 1:frame_interval:frame_length
        ...
        for n = 1:size(frame_ind,1)
            ...
            roi = single(movie(y-rhw:y+rhw,x-rhw:x+rhw,fr-f+1) - uint16(background(ind)));
            roi = roi/sum(roi,'all');
            temp_roi_array(:,:,frame_ind(n)) = roi;
        end
